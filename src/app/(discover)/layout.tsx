...
function NavAvatar({ name, url }: { name?: string | null; url?: string | null }) {
  ...
  const initials = parts.length >= 2
    ...
    : safe.slice(0, 2);
  return (
    <span className="flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold text-white uppercase ring-2 ring-white shadow-sm"
      ...
      <div className="bg-white/80 backdrop-blur-xl border-b border-[#EDE9E3]">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 h-16 flex items-center gap-5">
          ...
          <Link href="/" className="flex items-center gap-2.5 shrink-0 group">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg shadow-sm transition-transform group-hover:scale-105"
              style={{ background: "linear-gradient(135deg,#4A6741,#6B8F6E)" }}>
          ...
          <div className="flex items-center gap-3 ml-auto shrink-0">
            <div className="hidden sm:flex items-center gap-2.5">
              <NavAvatar name={profile?.fullName ?? user?.fullName} url={profile?.avatarUrl} />
            ...
            return (
              <Link key={href} href={href} onClick={onClose}
                className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-colors ${
                  active ? "bg-[#4A6741] text-white" : "text-[#4A4A4A] hover:bg-[#F0F5EE] hover:text-[#4A6741]"
                }`}>
